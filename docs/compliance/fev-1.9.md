...
| FEV19-SOAP-005 | WSDL habilitación, SHA `da576f…` | `UploadDocumentResponse`, `XmlParamsResponseTrackId` + SOAP 1.2 Fault | Conservar ZipKey, resultado/código/mensaje por documento, Fault multilenguaje, detalle, HTTP y XML original | `DianSoapResponseParser`, `Responses/*` | respuestas SOAP positiva/fault/negativas sintéticas según WSDL | `DianSoapResponseParserTest` | `locally_validated` |
| FEV19-SOAP-006 | WSDL habilitación, SHA `da576f…` | `GetStatus`, `DianResponse`, `ArrayOfstring` | Consultar por `trackId` y conservar errores, validez, códigos, mensajes, binarios Base64, claves, HTTP y XML original sin inferir aceptación | `GetStatusRequest`, `DianStatusClient`, `DianSoapResponseParser`, `DianResponse` | `get-status-response.xml` + SOAP Fault + Base64 inválido | `DianStatusClientTest` | `locally_validated` |
| FEV19-FILE-001 | Anexo FEV 1.9, SHA `1b4022…` | 6.5.7, pp. 303-304 | Nombre XML por tipo/NIT/PT/año/token hexadecimal explícito; no inferir incremento por Q-008 | `DianDocumentType::filePrefix`, `Fev19FileSequence`, `Fev19FileNameGenerator` | `submission/naming.json` positivo/negativo | `Fev19FileNameGeneratorTest` | `locally_validated` |
| FEV19-ZIP-001 | Anexo FEV 1.9, SHA `1b4022…` | 6.5.8, pp. 304-305 | Nombre ZIP; síncrono=1 XML, asíncrono=1..50 XML; mezcla de tipos permitida | `Fev19ZipPackageBuilder`, `Fev19SubmissionDocument` | nombres trazables + ZIP sintéticos en memoria | `Fev19ZipPackageBuilderTest` | `locally_validated` |
